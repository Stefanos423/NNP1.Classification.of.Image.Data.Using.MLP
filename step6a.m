function [accuracy precision recall] = step6a(TestData, TestDataTargets, TrainData, TrainDataTargets, neuronsFirstLevel, neuronsSecondLevel, BestTrain)
 neuronsFirstLevel neuronsSecondLevel
accuracy = [];
precision = [];
recall = [];

neural = newff(TrainData, TrainDataTargets, [neuronsFirstLevel neuronsSecondLevel], {'tansig' 'tansig' 'purelin'},BestTrain);
...
neural = newff(TrainData, TrainDataTargets, [neuronsFirstLevel neuronsSecondLevel], {'tansig' 'tansig' 'tansig'},BestTrain);
neural = train(neural, TrainData, TrainDataTargets);
TestDataOutput = sim(neural, TestData);
[accu(2,:),prec(2,:),rec(2,:)] = eval_Accuracy_Precision_Recall(TestDataOutput, TestDataTargets);

neural = newff(TrainData, TrainDataTargets, [neuronsFirstLevel neuronsSecondLevel], {'tansig' 'tansig' 'logsig'},BestTrain);
neural = train(neural, TrainData, TrainDataTargets);
TestDataOutput = sim(neural, TestData);
[accu(3,:),prec(3,:),rec(3,:)] = eval_Accuracy_Precision_Recall(TestDataOutput, TestDataTargets);

neural = newff(TrainData, TrainDataTargets, [neuronsFirstLevel neuronsSecondLevel], {'tansig' 'tansig' 'hardlim'},BestTrain);
neural = train(neural, TrainData, TrainDataTargets);
TestDataOutput = sim(neural, TestData);
